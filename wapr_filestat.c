...
static void fill_out_finfo(apr_finfo_t *finfo, struct stat *info,apr_int32_t wanted)
{
	...
	finfo->protection = wapr_unix_mode2perms(info->st_mode);
                                      ...
                                      wapr_file_t *thefile)
{
	...
	if (thefile->buffered)
	{
		apr_status_t rv = wapr_file_flush_locked(thefile);
		if (rv != APR_SUCCESS)
			return rv;
	}
...
apr_status_t wapr_file_info_get(apr_finfo_t *finfo, apr_int32_t wanted, wapr_file_t *thefile)
{
	struct stat info;
	int rc;
	...
	if (thefile->buffered)
	{
		apr_status_t rv = wapr_file_flush(thefile);
		if (rv != APR_SUCCESS)
		return rv;
	}
	rc = wrapper_sys_newfstat(thefile->filedes, &info);
...
apr_status_t wapr_file_perms_set(const char *fname, apr_fileperms_t perms)
{
	apr_status_t rc;
	mode_t mode = wapr_unix_perms2mode(perms);
...
apr_status_t wapr_file_attrs_set(const char *fname, apr_fileattrs_t attributes,
				apr_fileattrs_t attr_mask, apr_pool_t *pool)
{
	...
	status = wapr_stat(&finfo, fname, APR_FINFO_PROT, pool);
	if (status)
		return status;
	...
	return wapr_file_perms_set(fname, finfo.protection);
}

apr_status_t wapr_file_mtime_set(const char *fname, apr_time_t mtime,
				apr_pool_t *pool)
{
	apr_status_t status;
	apr_finfo_t finfo;

	status = wapr_stat(&finfo, fname, APR_FINFO_ATIME, pool);
	if (status)
		return status;
